...
# You can also redirect the standard output to go to another program or command,
# instead of the command line or a file.

# For this we'll introduce a new command, "head".

# The "head" command simply takes in the standard input and spits it back out
# the standard output, but only the first 10 lines.
...
# The "tail" command does the same thing but for the last 10 lines.
...
# You can also specify how many lines you want by using the "-n" command.
...
# There is no limit on how many times we can pipe something, though, so let's
# try to string the two together to get only the third item in the directory:
